          ...
          <div className="flex items-center space-x-2.5">
            ...
            <div className="relative">
              <button
                ...
                className={`px-3 py-1.5 rounded-xl border text-xs font-bold flex items-center space-x-1.5 transition-all shadow-2xs hover:opacity-90 ${modeBadgeColor}`}
              >
              ...
              {showModeMenu && (
                <div className="absolute right-0 mt-2 w-56 bg-white rounded-2xl border border-slate-200 shadow-xl p-1.5 z-50 animate-fade-in text-xs space-y-1">
                  <div className="px-2.5 py-1 font-bold text-slate-400 uppercase tracking-wider text-[10px]">
                    Switch Autonomy Mode
                  </div>
                  {[
                    { id: 'OBSERVE', label: 'OBSERVE (Read-Only)', desc: 'Zero side-effects, recommendations only', color: 'text-blue-700 hover:bg-blue-50' },
                    { id: 'ASSIST', label: 'ASSIST (Recommended)', desc: 'Autonomous + Human review on high risk', color: 'text-amber-800 hover:bg-amber-50' },
                    { id: 'AUTOPILOT', label: 'AUTOPILOT (Full Auto)', desc: 'Zero wait, 100% autonomous execution', color: 'text-emerald-800 hover:bg-emerald-50' }
                  ].map(m => (
                    <button
                      key={m.id}
                      onClick={() => {
                      ...
                      className={`w-full text-left px-3 py-2 rounded-xl transition-all ${m.color} ${currentMode === m.id ? 'bg-slate-100 font-extrabold ring-1 ring-slate-300' : 'font-semibold'}`}
                    >
                      <div className="flex items-center justify-between">
                        <span>{m.label}</span>
                        {currentMode === m.id && <CheckCircle2 className="w-3.5 h-3.5 text-blue-600" />}
                      </div>
                      <p className="text-[10px] text-slate-400 font-normal mt-0.5">{m.desc}</p>
                    </button>
                  ))}
                </div>
              )}
